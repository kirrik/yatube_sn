...
class TestPosts(TestCase):
    ...
    def setUp(self):
        ...
        self.user2 = User.objects.create_user(username='john', email='connor.j@skynet.com', password='09876')
        self.user3 = User.objects.create_user(username='terminator', email='terminator@skynet.com', password='56789')
    ...
    def test_new_post_pub(self):
        self.client.force_login(self.user)
        post = Post.objects.create(text="Текст тестового поста", author=self.user)
    ...
    def test_post_edit(self):
        self.client.force_login(self.user)
        post = Post.objects.create(text="Текст тестового поста 2", author=self.user)
    ...
    def test_post_with_img(self):
        self.client.force_login(self.user)
        post = Post.objects.create(text="Текст тестового поста 777", author=self.user)
        with open('media\posts\leo.jpg', 'rb') as fp:
            self.client.post(f'/{self.user.username}/{post.id}/edit/', {'text':'fred', 'image':fp})
    ...
    def test_upload_not_img_file(self):
        ...
        with open('media\posts\kirrik.txt', 'rb') as fp:
            response = self.client.post(f'/{self.user.username}/{post.id}/edit/', {'text':'fred', 'image':fp})
    ...
    def test_new_post_only_for_followers(self):
        ...
        response = self.client.get('/follow/')
        self.assertNotContains(response, "Текст тестового поста", status_code=200)
