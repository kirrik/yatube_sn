...
class TestPosts(TestCase):
    ...
    def test_only_auth_user_can_comment(self):
        self.client.force_login(self.user)
        user_post = Post.objects.create(text="Текст тестового поста", author=self.user)
        self.client.post(
        ...
        auth_response = self.client.get(f'/{self.user.username}/{user_post.id}/')
        self.assertContains(auth_response, "Текст комментария авторизованного пользователя", status_code=200)
        ...
        self.assertRedirects(not_auth_response, '/auth/login/?next=/sarah/1/comment/')
